...
### v0.4.0（已发布）
- ✅ 幻灯片模式（播放/暂停、间隔、末尾行为）
- ✅ 只读旋转翻转预览（R/Shift+R/H/V）
- ✅ EXIF 完整显示（相机/镜头/曝光/GPS）
- ✅ 菜单体验优化（单行文案、自适应宽度、省略号与悬停全文）
- ✅ 菜单截断安全与回归测试（中英、大字号、grapheme）

### v0.5.0（进行中，4 周节奏）
- Week 1：RAW MVP 骨架（Canon + Nikon），打通打开/浏览/失败兜底
- Week 2：单图编辑导出闭环（覆盖/另存、格式转换、质量参数）
- Week 3：批处理 MVP（重命名/导出、进度/取消/失败统计）
- Week 4：稳定性收口与发布（回归测试、文档同步、发布清单）

### v0.6.0（进行中，3 周节奏）
- Week 1：RAW 扩展（Sony + Fuji），覆盖 Viewer/Gallery/信息面板链路
- Week 2：稳定性与性能收敛（失败回退、混合目录性能优化）
- Week 3：最终回归、文档同步、发布清单
